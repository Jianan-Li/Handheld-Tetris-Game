...
byte blockStatus[16];  // 0: current movable piece; 1: dead blocks
  ...
  for (int i=0;i<4;i++) {
    for (int j=0;j<4;j++) {
      if (pieces[pieceIndex][j][i]>0) {
        ...
        if (row+j>=0 && ((blockStatus[row+j]>>(7-(col+i)))&1)==1) return true;  // checks overlap
      }
    }
  }
  return false;
...
boolean checkIfLanded() {
  for (int i=0;i<4;i++) {
    for (int j=0; j<4;j++) {
      if ((currentPieceRow+j>-1 && currentPieceRow+j<rows) && (currentPieceCol+i>-1 && currentPieceCol+i<cols)) {
        if (currentMatrix[currentPieceRow+j][currentPieceCol+i]>0 && ((blockStatus[currentPieceRow+j]>>(7-(currentPieceCol+i)))&1)==0 ) {
          if (currentPieceRow+j==bottomLimit) return true;            // if the piece has dropped on the bottom
          else if (((blockStatus[currentPieceRow+j+1]>>(7-(currentPieceCol+i)))&1)==1) return true;  //if there is any deadblock just below the piece
        }
      }
    }
  }
  ...
  for (byte i=0; i<4; i++) {
    for (byte j=0; j<2; j++) {
      if (piecesGenerated[pieceIndex][j+2][i]>0) {
        if (((blockStatus[j]>>(7-(i+2)))&1)==1) return false;
      }
    }
  }
  for (byte i=0; i<4; i++) {
...
void convertToDeadBlock() {
  for (byte i=0;i<rows;i++) {
    for (byte j=0;j<cols;j++) {
      if (currentMatrix[i][j]>0 && blockStatus[i]) {
        blockStatus[i]=blockStatus[i]|(1<<(7-j));
      }
    }
  }
}
...
void setup() {
  ...
  OCR2A = 13;  // = (16*10^6) / (4480.0Hz*256) - 1 (must be < 256)
...
//  }
//  
  // refresh the display (one run at a time, 7 runs per col)
  refreshDisplay();
...
// 0: active; 1: dead
byte getBlockStatus(i,j) {
  return (blockStatus[i]>>(7-j)) & 1;
}

void setBlockStatus(i,j,val) {
  byte currentStatus = blockStatus[i];
  byte back = 
